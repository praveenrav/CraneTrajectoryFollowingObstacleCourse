function list = sensplot(seq,fmin,fmax,zeta,points)

% sensplot  Plot the residual over range of frequencies
% The residual vibration is NOT normalized by the "unshaped" vibration,
% rather it is normalized by the endpoint value.
...
zeta^2)*seq(i,1));
  ...
  list(nn+1,2) = freq/sqrt(1-zeta^2)*exp(-zeta*freq*tn)*sqrt(sintrm^2+costrm^2);
end
plot(list(:,1),list(:,2))
